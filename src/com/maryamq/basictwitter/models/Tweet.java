...
public class Tweet extends Model implements Serializable {
	...
	public void persist() {
		...
		this.save();
		// check
		boolean notNull = this.getTweet(this.uid) != null;
	...
	public static Tweet getTweet(long id) {
		List<Tweet> savedTweets = new Select().from(Tweet.class).where("tweetId=?", id)
				.execute();
	...
	public static List<Tweet> getMentionedTweets() {
		List<Tweet> list = new Select().from(Tweet.class)
				.where("user_mentioned=?", Boolean.valueOf(true))
	...
	public static List<Tweet> getHomeTimelineTweets() {
		List<Tweet> list = new Select().from(Tweet.class)
				.where("is_home_timeline=?", Boolean.valueOf(true))
				.orderBy("tweetId DESC").execute();
		return list;
	}
	...
	public static Tweet fromJSON(JSONObject json, boolean persist) {
		...
		try {
			...
			tweet.inReplyToUserId = json.isNull("in_reply_to_screen_name") ? "" : json
			...
			if (!json.isNull("user_mentions")) {
				...
				if (TwitterApplication.getRestClient().getAccount() != null) {
					long accountId = TwitterApplication.getRestClient().getAccount()
							.getUid();
